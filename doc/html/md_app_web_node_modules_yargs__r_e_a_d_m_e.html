...
<h1>methods </h1>
<p>By itself,</p>
<p>````javascript require('yargs').argv `````</p>
...
<p>`.string('_')` will result in non-hyphenated arguments being interpreted as strings, regardless of whether they resemble numbers.</p>
<h2>.array(key) </h2>
<p>Tell the parser to interpret <code>key</code> as an array. If `.array('foo')<code>is set, </code>&ndash;foo foo bar<code>will be parsed as</code>['foo', 'bar']<code>rather than as</code>'bar'`.</p>
...
<p>`{ _: [], token: '-my-token', '$0': 'node test' }`</p>
