...
app.controller("OrgaWizardCtrl", function(
  $scope,
  $http,
  $timeout,
  $uibModal,
  ...
  $scope.governance_types = {
    dao: {
      pros: [
        ...
        "Withdraw your funds at all time",
        "No membership required"
      ],
      cons: [
        "Less reactive than a centralized structure",
        "Permissions are non-customisable",
        "Tokens are frozen while a proposal you voted for is still debating"
      ],
      desc:
    ...
    ngo: {
      ...
      cons: [
        "The perfect fit for public organisations",
    ...
    public_company: {
      ...
      cons: [
        "Perfect for small and large businnesses wanting to go public",
        "Buy and sell your company shares",
        "Highly secured with the use of curators"
      ],
      desc:
    ...
    entreprise: {
      pros: [
        ...
        "Permissions are highly customizable",
        "Very easy to manage"
      ],
      cons: [
        "Perfect fit for individuals wanting full control over the structure",
        "Members need to trust the administrators"
      ],
      desc:
  ...
  $scope.$watch("orga_form.gov_model", function() {
    ...
    $scope.orga_form.rights = $.extend(
      {},
      $scope.governance_types[$scope.orga_form.gov_model]["rights"]
    );
  });
  ...
  $rootScope.addRight = function() {
    if (
      $("#newOrgaRight")
        .val()
        .trim() &&
      !$scope.orga_form.rights[$("#newOrgaRight").val()]
    ) {
      $scope.orga_form.rights[$("#newOrgaRight").val()] = $.extend(
        {},
        $scope.governance_types[$scope.orga_form.gov_model].rights["default"]
      );
    }
  };

  $scope.addInvitedUser = function() {
    if (!$scope.orga_form.invited_users) $scope.orga_form.invited_users = {};
    if ($scope.selected_user.originalObject.category != "user") {
      $rootScope.toogleInfo("You can only invite user!");
      return false;
    }
    $scope.orga_form.invited_users[$scope.selected_user.originalObject._id] =
      $scope.selected_user.originalObject;
    $scope.orga_form.invited_users[$scope.selected_user.originalObject._id][
      "tag"
    ] =
      "member";
  ...
  $scope.tagChangedForUser = function(user_id) {
    $scope.orga_form.invited_users[user_id]["rights"] =
      $scope.governance_types[$scope.orga_form.gov_model]["rights"][
        $scope.orga_form.invited_users[user_id]["tag"]
      ];
  };
        ...
        $scope.completeBlockchainAction(
          ...
          function(data) {
            ...
            if (uploaderImages.queue.length != 0) {
              uploaderImages.queue[0].formData.push({
                orga_id: data.data._id,
                type: uploaderImages.queue[0].file.type
              });
            }
