...
from core.utils import normalizeAddress
...
class ProjectDocument(Document):
	...
	def join(self, user, tag="member", password=None, local=False):
		...
		tx_hash = self.registry.call('registerToProject', local=local, from_=user.get('account'), args=[self.get('address'), user.get('account'), tag], password=password)
