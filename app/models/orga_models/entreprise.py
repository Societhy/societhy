...
class Entreprise(Organization):
    ...
    def register(self, tx_receipt, callback_data=None):
        ...
        tx_hash = self.join(
            from_, tag="owner", password=callback_data.get('password'), local=False)
        if not tx_hash:
        ...
        for item in self.get('invited_users'):
            ...
            from_.unlockAccount(password=callback_data.get('password'))
            self.allow(from_, user.get('account'), None,
                       password=callback_data.get('password'), local=False)
            user.save()

        resp = {"name": self["name"], "_id": str(self["_id"])}
        resp.update({"data": {k: str(v) if type(v) ==
                              objectid.ObjectId else v for (k, v) in self.items()}})
        notif = models.notification.NotificationDocument({
