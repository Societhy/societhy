...
def makeTopics(signature, *args):
    ...
    ret = list()
    if signature:
        signature = to32bytes(keccak_256(
...
class Event:
    ...
    def __init__(self, tx_hash=None, users=[], callbacks=None, callback_data=None, mail=None):
        ...
        if isinstance(users, list):
            self.users = [user if isinstance(user, str) else user.get(
                'socketid') for user in users]
        elif isinstance(users, str):
            self.users = [users]
        else:
            self.users = [users.get('socketid')] if users.get(
                'socketid') is not None else None
    ...
    def process(self):
        ...
        for cb in self.callbacks:
            self.notifyUsers(self.users, cb())
        return self
    ...
    def process(self):
        ...
        if self.event_abi and len(self.logs) >= 1:
            ...
            while i <= len(raw_data):
                try:
                    decoded_data.append(decode_hex(
                        raw_data[i - 32:i]).decode("ascii"))
                except:
                    decoded_data.append(decode_hex(raw_data[i - 32:i]))
                i += 32
            self.logs[0]["decoded_data"] = [line for line in [line.strip(
                '\x00').strip() for line in decoded_data] if len(line) > 1]
        if self.mail:
            for user in self.mail['users']:
                from models.notification import notifyToOne
                notifyToOne(self.mail['sender'], user,
                            'NewMember', self.mail['subject'])
        for cb in self.callbacks:
